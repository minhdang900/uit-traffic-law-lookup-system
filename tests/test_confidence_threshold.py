...
----------------------------------------------
``src/traffic_law/retrieval/dense.py`` hiện thực bộ lọc miền bằng dense
...
OUT_OF_DOMAIN = ["cách nấu phở bò", "giá vàng hôm nay", "lập trình Python cơ bản"]
...
class TestResultContract:
    ...
    def test_not_found_key_always_present(self, system):
        """Giao diện và bộ đánh giá đọc khoá này; thiếu nó sẽ ném KeyError."""
        for query in IN_DOMAIN + OUT_OF_DOMAIN:
            kq = system.ask(query)
            assert "not_found" in kq, f"Thiếu khoá với truy vấn {query!r}"
            assert isinstance(kq["not_found"], bool)

    @pytest.mark.parametrize("query", IN_DOMAIN)
    def test_valid_query_is_not_rejected(self, system, query):
        kq = system.ask(query)
        assert kq["not_found"] is False
        assert kq["concepts"] or kq["rules"] or kq["violations"]

    def test_none_of_the_120_questions_is_wrongly_rejected(self, system, qa_set):
        ...
        assert rejected == [], f"{len(rejected)}/120 câu bị từ chối oan: {rejected[:10]}"


class TestSupplementThreshold:
    """Ngưỡng chỉ chi phối tri thức BỔ SUNG, không bao giờ chặn kết quả chính."""

    def test_threshold_sits_between_measured_distributions(self):
        """Trần truy vấn rác 0.3225 < ngưỡng < phân vị 5% truy vấn hợp lệ 0.45."""
        assert 0.3225 < SUPPLEMENT_THRESHOLD < 0.45

    def test_supplementary_knowledge_meets_threshold(self, system):
        for query in IN_DOMAIN:
            kq = system.ask(query)
            for kind in ("concepts", "rules", "violations"):
                duoi = [x for x in kq[kind]
                        if x.get("supplementary") and x.get("score", 0) < SUPPLEMENT_THRESHOLD]
                assert not duoi, f"{query!r}: {len(duoi)} mẩu bổ sung dưới ngưỡng"

    def test_threshold_does_not_regress_metrics(self, system, qa_set):
        """Chốt chặn hồi quy: Top-1 phải giữ mức đường cơ sở 76,67%."""
        dung = 0
        for m in qa_set:
            kq = system.ask(m["cau_hoi"], top_k=5)
            kind = m["loai_tri_thuc"]
            khoa = {"concept": "concepts", "rule": "rules"}.get(kind, "violations")
            tra_ve = [x["id"] for x in kq[khoa]]
            if tra_ve and tra_ve[0] in m["id_tri_thuc_dung"]:
                dung += 1
        top1 = dung / len(qa_set)
        assert top1 >= 0.76, f"Top-1 tụt xuống {top1:.2%} (đường cơ sở 76,67%)"


class TestMeasuredLimitations:
    ...
    def test_should_reject_out_of_domain_query(self, system, query):
        kq = system.ask(query)
        assert kq["not_found"] is True, (
            f"{query!r} vẫn trả về {len(kq['behavior'])} hành vi vi phạm")

    @pytest.mark.xfail(strict=True, reason=(
        "12/120 câu hợp lệ cũng rút được 0 keyphrase, nên không dùng riêng tín "
        "hiệu keyphrase để lọc truy vấn ngoài lĩnh vực."))
    def test_should_extract_keyphrase_for_every_valid_question(self, system, qa_set):
        without = [m["id"] for m in qa_set
                    if not system.engine.analyzer.analyze(m["cau_hoi"])["keyphrase"]]
        assert without == [], f"{len(without)}/120 câu không rút được keyphrase nào"
