...
def tach_so_tien(s: str) -> list[int]:
    ...
    for m in re.finditer(r"(\d[\d.,]*)\s*(tỷ|tỉ|triệu|nghìn|ngàn|k|đồng|vnđ|vnd)?", s):
        raw, don_vi = m.group(1), (m.group(2) or "")
        if don_vi not in _HE_SO_TIEN:
            continue
        try:
            gia_tri = float(raw.replace(".", "").replace(",", "."))
        except ValueError:
            continue
        out.append(int(gia_tri * _HE_SO_TIEN[don_vi]))
    return out
