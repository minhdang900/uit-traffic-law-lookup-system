...
if TYPE_CHECKING:  # pragma: no cover - chi phuc vu kiem kieu
    from traffic_law.reasoning.engine import IndexedKnowledgeBase
...
SAFE_DOMAIN_THRESHOLD = 0.45

#: Ngưỡng loại được 100% truy vấn rác — KHÔNG dùng mặc định vì làm oan 3/120 câu.
STRICT_DOMAIN_THRESHOLD = 0.52

CACHE_DIR = os.environ.get(
    "TRAFFIC_LAW_CACHE", os.path.join(os.path.expanduser("~"), ".cache", "traffic-law"))
...
class DomainFilter:
    ...
    def _cache_path(self) -> str:
        ...
        return os.path.join(CACHE_DIR, f"nhung_{khoa}.npy")
    ...
    # -------------------------------------------------------------- cham diem
    def domain_score(self, query: str) -> float:
    ...
    def keyphrase_count(self, query: str) -> int:
        ...
        kps = self.analyzer.extract_keyphrases(query)  # type: ignore[no-untyped-call]
        return len(kps)
    ...
    def is_out_of_domain(self, query: str) -> bool:
        ...
        """
        if self.keyphrase_count(query) > 0:
            return False
        return self.domain_score(query) < self.threshold
